...
var (
	contractHash = sha256Hash([]byte(contract.ContractBin[:]))
)
...
func unpackContractInputParams(abi abi.ABI, tx *types.Transaction) (struct {
	LockDuration uint
	...
	if err != nil {
		return params, fmt.Errorf("failed to get method using its parsed id: %v", err)
	}

	fmt.Println(hex.EncodeToString(txData[4:]))

	// unpack and return the params
	err = method.Inputs.Unpack(&params, txData[4:])
	if err != nil {
		return params, fmt.Errorf("failed to unpack method's input params: %v", err)
	}
	fmt.Println(params.LockDuration)
	return params, nil
}
...
func (cmd *auditContractCmd) runCommand(sct swapContractTransactor) error {
	...
	// TODO: DEGBUG WHY lock duration is 0 :()
	// TODO: DEBUG WHY Receipient Address is nil

	// compute the locktime
	fmt.Println(block.Time().Int64(), params.LockDuration)
	lockTime := time.Unix(block.Time().Int64()+int64(params.LockDuration), 0)
